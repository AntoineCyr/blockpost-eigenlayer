...
# start-chain-and-deploy-blockpost-avs: build-contracts deploy-contracts-to-anvil-and-save-state start-anvil-chain-with-el-and-avs-deployed
...
send-fund: ## sends fund to the operator saved in tests/keys/test.ecdsa.key.json
	cast send 0x860B6912C2d0337ef05bbC89b0C2CB6CbAEAB4A5 --value 1000ether --private-key 0x2a871d0798f97d79848a013d4936a73bf4cc922c825d33c1cf7073dff6d409c6
...
register-operator: ## start operator (part of quickstart)
	npx ts-node ./operator/registerOperator.ts

start-operator: ## start operator (part of quickstart)
	npx ts-node ./operator/monitorTask.ts
